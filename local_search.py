from helpers import *
from data.model import Route, OperatorType
    ...
    for i in range(len(route.job_ids) - 1):
        for j in range(i + 1, len(route.job_ids)):
            ...
            if new_route.delivery_duration < route.delivery_duration:
                ...
                if output_log:
                    print('{0} improvement\t-> {1} seconds.'
                          .format(OperatorType.TwoOpt, improvement))
                return True
    return False
...
def switch(data, routes, operator_type):
            ...
            if r1 != r2:
                ...
                for j1 in range(len(route1.job_ids)):
                    for j2 in range(len(route2.job_ids)):
                        ...
                        elif operator_type == OperatorType.Relocate:
                            # get job to relocate
                            job1 = [v for v in data.jobs if v.id == route1.job_ids[j1]][0]
                            # create copies of routes
                            route1_copy = route1.copy()
                        ...
                        if new_total_delivery_duration < current_total_delivery_duration and \
                            ...
                            print('{0} improvement\t-> {1} seconds.'
                                  .format(operator_type, improvement))
                            return True
    return False


def apply_switch(data, routes, operator_type):
    improved = True
    while improved:
        improved = switch(data, routes, operator_type)
...
def local_search(data, solution):
    ...
    while improved:
        ...
        apply_switch(data, solution.routes, OperatorType.Exchange)
